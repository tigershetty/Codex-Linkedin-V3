...
- **AI-still backup lane has its own playbook → `references/ai-still-prompt-learnings.md`** (read before writing a GPT Image 2 prompt). Hard-won rules from the Ep35 iteration:
  - **Framework first, prompt second** — when an AI-still infographic looks "good but generic," change the *framework* (the argument the shape makes), not the adjectives. Decompose the message into its claims; each claim → a different shape (see `layout-frameworks-intelligence.md` §1 selection method).
  - **"Dramatic / less AI-like / designer-made" ≠ cinematic** — it means *cleaner and conceptually sharper*. Camera angle, raking light, chiaroscuro, "mood" produce a moody photographic scene, the opposite of a crisp infographic. Frame prompts as design-forward information graphics (FT/Economist), flat-on, even light.
  - **Reference anchors are load-bearing** — always attach the **4 white-bg anchors + logo (logo LAST)**; logo-only makes the engine fall back to a generic prior. The white-bg anchors beat the deep-blue `brand-anchor-v1.webp` (muddy colour).
  - **GPT Image 2 cannot do 4:5** — use **3:4** (quality high, resolution 4k). Internal engine name "videotape-alpha" is normal; the nano model IDs get remapped on the backend.
