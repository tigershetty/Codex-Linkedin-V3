...
|---|---|---|---|
| Audience | Experts AND non-practitioners | SC Practitioners (daily tasks) + SC Leaders (high-stakes decisions) | VP Supply Chain, COO, Directors |
| Vocabulary | Plain language, technical terms explained | Action-oriented, tool-specific, no hype | Industry-grade, numbers-heavy |
| Research | None — topics pre-planned in monthly topic bank | None — topics pre-planned in monthly topic bank | Full /research with evidence ledger |
| Hooks | Metaphor-led, accessible | Practitioner: task + time frame. Leader: time pressure + competitiveness tension | Data-led, case-study anchored |
| Caption | 150–300 words, educational tone | 220–320 words, instructional, action-first | 200–350 words, 7-part structure with stats |
| Control gates | None (single-step) | None (single-step, 2 posts per run) | CG1 (topic) + CG2 (hook) |
| Visual | Gemini with 101 brand kit template | Gemini — workflow, prompt template, or decision framework visual | Gemini with deep-dive visual DNA |
| Frequency | 2/week (Posts 1+2 in weekly sub-topic) | 2/week (Posts 3+4 in weekly sub-topic) | Paused |
| Output files | `101-copy.md` | `ai-for-sc-practitioner.md` + `ai-for-sc-practitioner-pdf.md` + `ai-for-sc-leader.md` + `ai-for-sc-leader-pdf.md` | `content.md` + `gemini-prompt.md` |
...
```
/ai-for-sc [week]              ← generates BOTH posts (Practitioner + Leader)
/ai-for-sc [week] practitioner ← generates SC Practitioner post only (Post 3)
/ai-for-sc [week] leader       ← generates Leader post only (Post 4)
```

Generates SC Practitioner post + Leader post from the monthly topic bank. Each post outputs a LinkedIn caption AND a PDF draft. Both posts cover the same weekly sub-topic at different depths.

Before generating, the skill loads: `tiger-voice.md`, `references/published-voice.md`, and `references/101-voice.md` — all three voice references apply to maximise hook quality and caption intelligence.

| Command | Trigger | Output files |
|---|---|---|
| `/ai-for-sc [week]` | Week number from monthly topic bank | `ai-for-sc-practitioner.md` + `ai-for-sc-practitioner-pdf.md` + `ai-for-sc-leader.md` + `ai-for-sc-leader-pdf.md` |
| `/ai-for-sc [week] practitioner` | SC Practitioner post only | `ai-for-sc-practitioner.md` + `ai-for-sc-practitioner-pdf.md` |
| `/ai-for-sc [week] leader` | Leader post only | `ai-for-sc-leader.md` + `ai-for-sc-leader-pdf.md` |

**Reference files (AI for SC only):**
| File | Purpose |
|---|---|
| `~/Claude Nano /tiger-voice.md` | Master voice DNA — loaded first, applies to all AI for SC output |
| `references/published-voice.md` | Deep dive voice markers — hook quality bar, "NOT THIS" list |
| `references/101-voice.md` | 101 voice — accessible register for the SC concept layer |
| `data/ai-for-sc-series-tracker.md` | Published episodes log — tracks Practitioner + Leader posts |
...
- **AI for SC: PDF draft is always generated alongside the post** — a post file without a matching PDF file is incomplete; flag at session start
...
- **AI for SC: SC Practitioner hook requires a specific time frame** — no time frame = not a valid Practitioner hook
- **AI for SC: Leader hook requires BOTH time pressure AND competitiveness tension** — one without the other is not a valid Leader hook
...
- **AI for SC — SC Practitioner**: Planners, demand managers, procurement officers, SC analysts. Daily task execution. Hook from daily friction + specific time frame. One tool, one task, one paste-ready prompt.
- **AI for SC — Leader**: SC Directors, VPs, COOs, S&OP leads — 10–20+ years experience. High-stakes decisions under time pressure. Two pain points: (1) closing the AI literacy gap vs. younger colleagues; (2) speed to insight before a decision. Hook combines time pressure + competitiveness tension. Not tech-native — needs practical entry points, not theory.
