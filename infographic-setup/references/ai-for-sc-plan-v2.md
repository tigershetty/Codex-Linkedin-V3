...
- **Visual**: Dashboard Preview — scorecard template showing criteria columns and supplier rows
...
- **Visual**: Option Scoreboard — Supplier A / Supplier B / Supplier C, cost components as rows
...
- **Visual**: Formula Tree — inputs (setup cost, demand, holding cost) → calculation → output (optimal batch size + cost comparison at 3 batch sizes)
...
- **Visual**: Before/After Split — naive baseline vs. adjusted forecast with FVA score and implication
...
- **Visual**: Diverging Paths — OT / Weekend shift / Outsource, cost + lead time + risk per scenario
...
- **Visual**: Formula Tree — inputs → revenue at risk calculation → cost breakdown (stockout vs. overstock)
...
- **Visual**: Segmentation Matrix — variability vs. volume matrix with quadrant labels
...
- **Visual**: Tracker Sheet — SKU tiers as rows, reorder triggers highlighted
...
- **Visual**: Option Scoreboard — Fixed Qty / Fixed Interval / Min-Max, cost + service level + volatility as rows
...
- **Visual**: Option Scoreboard — Carrier A / Carrier B / Carrier C, cost components and performance score
...
- **Visual**: Formula Tree — MRP inputs (demand, stock on hand, safety stock, lead time) → manual calculation → comparison with ERP output
...
- **Visual**: Working Capital Bridge — inputs (DIO, revenue, inventory value) → 5-day reduction calculation → working capital freed
...
- **Visual**: Before/After Split — current state costs vs. investment + return, with payback timeline
...
- **Visual**: Working Capital Bridge — suppliers as rows, current terms vs. extended terms, working capital impact per supplier and total
...
- **Visual**: Diverging Paths — Optimistic / Base / Downside, SC implication and team response per scenario
...
| Ep | Week | Role | Tool | Use Case (short) | Visual Format |
|---|---|---|---|---|---|
| 01 | W21 | Purchaser | Claude | RFQ with evaluation criteria and scoring matrix | Blueprint Draft |
| 02 | W21 | Category Manager | ChatGPT | Should-cost model for negotiation | Cost Anatomy |
| 03 | W22 | Purchaser | Claude | Contract review — flag 5 risk clauses | Clause Reveal |
| 04 | W22 | Category Manager | Gemini | Market pricing research to challenge supplier | Signal Scan |
| 05 | W23 | Category Manager | Copilot | Supplier scorecard for QBRs in Excel | Dashboard Preview |
| 06 | W23 | Purchaser | Claude | Supplier price increase counter-arguments | Negotiation Table |
| 07 | W24 | Purchaser | Claude | Maverick spend analysis | Spend Distribution |
| 08 | W24 | Category Manager | ChatGPT | TCO comparison across 3 supplier quotes | Option Scoreboard |
| 09 | W25 | Supply Planner | Claude | Supplier flexibility matrix | Supplier Risk Card |
| 10 | W25 | Category Manager | Gemini | Supplier risk profile | Network Map |
| 11 | W26 | Demand Planner | Claude | Demand decomposition (trend / seasonality / noise) | Forecast Anatomy |
| 12 | W26 | S&OP Analyst | Copilot | Demand review tracking dashboard | Dashboard Preview |
| 13 | W27 | Production Planner | Claude | Production sequence — minimise changeover | Production Queue |
| 14 | W27 | Supply Planner | ChatGPT | Optimal batch size calculation | Formula Tree |
| 15 | W28 | Supply Planner | Claude | SKUs for demand sensing vs. monthly forecasting | Parallel Tracks |
| 16 | W28 | Demand Planner | ChatGPT | Forecast value-add vs. naive baseline | Before/After Split |
| 17 | W29 | S&OP Analyst | Claude | S&OP demand review narrative | Executive Brief |
| 18 | W29 | Demand Planner | Copilot | Forecast accuracy tracking by product family | Dashboard Preview |
| 19 | W30 | Production Planner | Claude | Bottleneck identification by cycle time | The Bottleneck |
| 20 | W30 | Supply Planner | ChatGPT | 3 capacity scenarios for peak season | Diverging Paths |
| 21 | W31 | Demand Planner | Claude | Promotion lift analysis | Variance Zone |
| 22 | W31 | S&OP Analyst | Gemini | Leading indicators for 8-week forecast | Correlation Map |
| 23 | W32 | Supply Planner | Claude | Safety stock parameter audit | Inventory Stack |
| 24 | W32 | Demand Planner | ChatGPT | Revenue at risk from forecast inaccuracy | Formula Tree |
| 25 | W33 | Supply Planner | Claude | Lead time doubling — stockout risk model | Disruption Timeline |
| 26 | W33 | Demand Planner | Copilot | SKU segmentation by variability (MTS vs. MTO) | Segmentation Matrix |
| 27 | W34 | Warehouse Manager | Claude | Warehouse utilisation by zone — slotting audit | Warehouse Zone Map |
| 28 | W34 | Supply Planner | Copilot | Inventory replenishment calendar by SKU tier | Tracker Sheet |
| 29 | W35 | Warehouse Manager | Claude | Slotting strategy — golden zone, bulk, offsite | Warehouse Zone Map |
| 30 | W35 | Supply Planner | ChatGPT | Replenishment strategy comparison — 3 methods | Option Scoreboard |
| 31 | W36 | Logistics Coordinator | Claude | Exception management — transport admin compression | Triage Funnel |
| 32 | W36 | Transport Specialist | ChatGPT | Lane-level transport savings (8–12%) | Network Map |
| 33 | W37 | Logistics Coordinator | ChatGPT | Carrier quote comparison — total transport cost | Option Scoreboard |
| 34 | W37 | Transport Specialist | Copilot | Carrier performance dashboard | Dashboard Preview |
| 35 | W38 | Logistics Coordinator | Claude | 6-month transport spend by lane | Spend Distribution |
| 36 | W38 | Supply Planner | ChatGPT | Bottleneck identification across SC layers | The Bottleneck |
| 37 | W39 | Supply Planner | Claude | Peak season capacity options comparison | Decision Crossroads |
| 38 | W39 | Warehouse Manager | Copilot | Throughput rate per labour hour by shift | Dashboard Preview |
| 39 | W40 | Logistics Coordinator | Claude | Disruption response playbook (top 3 suppliers) | Playbook Page |
| 40 | W40 | Supply Planner | ChatGPT | Supply stress-test: demand +25%, 2 suppliers fail | Stress Test Model |
| 41 | W41 | SC Analyst | Claude | ERP data gap mapping for forecasting | The Blind Spot |
| 42 | W41 | Supply Planner | Copilot | Manual MRP check in Excel | Formula Tree |
| 43 | W42 | Logistics Coordinator | Gemini | Trade compliance — HS code in 60 seconds | Decision Tree |
| 44 | W42 | SC Manager | Claude | Control tower exception management template | Control Tower View |
| 45 | W43 | SC Analyst | Copilot | Weekly SC KPI dashboard (5 metrics) | Dashboard Preview |
| 46 | W43 | Demand Planner | Claude | Planning data quality audit | The Blind Spot |
| 47 | W44 | Supply Planner | Claude | VMI replenishment rules document | Playbook Page |
| 48 | W44 | SC Manager | Copilot | IBP/S&OP data handshake template | Dashboard Preview |
| 49 | W45 | Category Manager | Claude | Annualised cost of procurement problem | Cost Lever |
| 50 | W45 | Supply Planner | ChatGPT | Working capital from 5-day inventory reduction | Working Capital Bridge |
| 51 | W46 | Category Manager | Claude | Supplier consolidation cost model | Waterfall Bar |
| 52 | W46 | SC Manager | ChatGPT | Cost of inaction model | Before/After Split |
| 53 | W47 | SC Manager | Claude | Executive summary for SC investment | Executive Brief |
| 54 | W47 | Category Manager | Copilot | Payment terms extension model (30→60 days) | Working Capital Bridge |
| 55 | W48 | Supply Planner | Claude | Month-by-month payback model | Payback Arc |
| 56 | W48 | SC Manager | Copilot | SC risk register in Excel | Dashboard Preview |
| 57 | W49 | Demand Planner | Claude | Personal prompt library — 5 forecasting prompts | Prompt Card |
| 58 | W49 | Logistics Coordinator | Copilot | Team SC KPI tracker template | Dashboard Preview |
| 59 | W50 | SC Analyst | Claude | 1-page AI workflow guide for team | Prompt Card |
| 60 | W50 | SC Manager | Gemini | Benchmark research — how SC teams use AI | Signal Scan |
| 61 | W51 | Supply Planner | Claude | Year-end SC performance narrative | Executive Brief |
| 62 | W51 | SC Manager | ChatGPT | 3 scenarios for 2027 SC challenges | Diverging Paths |
| 63 | W52 | SC Manager | Claude | Q1 2027 AI priority list | Decision Crossroads |
| 64 | W52 | Demand Planner | Copilot | 52-week SC planning calendar template | Dashboard Preview |
