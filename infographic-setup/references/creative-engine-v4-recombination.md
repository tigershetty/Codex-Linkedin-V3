...
- `creative-review/signature-system-v3.md` is the active brand authority. The brief must select a
  visual family from the reader tension and mechanism. A high-density Visual Framework can use an
  exact Figma social visual when the useful object is a map, comparison, diagnostic, taxonomy,
  decision path, or data relationship. Field Guide Notebook is reserved for a durable working
  reference that passes its specialist admission test. Operating Studio and Cobalt Grid are
...
3. Standard: keep the selected route in `post-card.md` and move directly to the selected visual
   surface and complementary caption. Flagship: build `creative-brief-lite.md` from the selected
   recombination.
4. Use the image engine for a selected image-led social visual. Use a deterministic Figma master as
   the final LinkedIn visual when a high-density Visual Framework needs exact type and geometry. Use
   HTML/SVG, spreadsheet, document, or another surface when the separate artifact adds more utility
   than a still. For a Field Guide, use its specialist Figma review contract; no renderer is the
   creative strategy.
...
- Use the selected visual family and Signature System V3. Show the reader tension
