// TypeScript → JavaScript faithful conversion.
// Preserves original class ordering (AnimationController before Star),
// factory-function pattern in setupRandomGenerator, and bracket notation
// controller['cameraZ'] / controller['viewZoom'] from Star.render().
// Only change from the TS original: Tailwind classNames → inline styles
...
// ── AnimationController ───────────────────────────────────────────
// Note: AnimationController is declared before Star (matching original order).
// This is safe because `new Star()` is only called inside methods at runtime,
// by which point Star is fully evaluated.
...
class Star {
  ...
  render(p, controller) {
    ...
    if (q > 0) {
      ...
      // Bracket notation preserved from original TS (controller['cameraZ'] was
      // the TS workaround for accessing private fields; semantically identical
      // to controller.cameraZ in JavaScript)
...
export function SpiralAnimation() {
  ...
  // Original used Tailwind: className="relative w-full h-full"
  // and className="absolute inset-0 w-full h-full"
  // Replaced with inline styles (project has no Tailwind)
  return (
